...
export function MarkdownTablePage() {
  ...
  const [markdownAlignments, setMarkdownAlignments] = useState<
    ('left' | 'center' | 'right')[]
  >([])
  ...
  const getColumnAlignments = () => {
    ...
    const alignments = cellFormats[0].map((_, colIndex) => {
      for (let rowIndex = 0; rowIndex < cellFormats.length; rowIndex++) {
        ...
        if (alignment !== 'left') {
          return alignment === 'inherit' || alignment === 'justify'
            ? 'left'
            : alignment
        }
      }
  ...
  const handleGenerateMarkdown = () => {
    ...
    const valuesEqual =
      JSON.stringify(markdownValues) === JSON.stringify(values)
    const currentAlignments = getColumnAlignments()
    const alignmentsEqual =
      JSON.stringify(markdownAlignments) === JSON.stringify(currentAlignments)
              ...
              <>
                <Button
                  ...
                  startIcon={
                    <VisibilityIcon
                      sx={{
                        animation: isAnimatingPreview
                          ? 'previewPulse 0.3s ease-in-out'
                          : 'none',
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                <Box sx={{ position: 'relative' }}>
                  ...
                  >
                    <Tooltip
                      title={
                        isCopied ? 'Copied!' : 'Copy markdown table syntax'
                      }
                      placement="left-end"
                      arrow
