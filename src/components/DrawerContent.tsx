...
function NavigationButton({ isSubItem, path, selected, onClick, children }: NavigationButtonProps) {
        ...
        <Box sx={{ p: 2 }}>
            <List sx={{ padding: 0 }}>
                {NAVIGATION_ITEMS.map(({ path, label, icon, showDividerBelow, subItems }) => {
                    const isSelected = !subItems && (path === "/" ? location.pathname === "/" : location.pathname === path);
